...
function Comment({ message, mockUser }) {
  const currentUser = JSON.parse(localStorage.getItem('user'));
  const { user } = mockUser ?? useUser(message.user);
  ...
  if (currentUser.id !== user?.user) {
    return (
      <div className={styles.commentContainer}>
        ...
        <p className={styles.commentText}>{message.text}</p>
      </div>
    );
  } else {
    return (
      <div className={styles.myCommentContainer}>
        <p className={styles.myCommentText}>{message.text}</p>
      </div>
    );
  }
}
